...
export const PATCH = withAuth<Params>(async (req: NextRequest, { params }) => {
  try {
    ...
    const existing = await prisma.partStatus.findFirst({ where: { id: statusId, deletedAt: null } });
    ...
    const updated = await prisma.partStatus.update({
      where: { id: statusId },
      data,
    });
    ...
    logger.error("Part status update error", error as Error, { path: "/api/part-statuses/[id]", method: "PATCH" });
    return serverError();
  }
...
export const DELETE = withAuth<Params>(async (_req: NextRequest, { params }) => {
  try {
    ...
    await prisma.partStatus.update({ where: { id: statusId }, data: { deletedAt: new Date() } });
    return json({ ok: true });
  } catch (error) {
    logger.error("Part status delete error", error as Error, { path: "/api/part-statuses/[id]", method: "DELETE" });
    return serverError();
  }
}, { requiredPermissions: ["parts.manage"] });
