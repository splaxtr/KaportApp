...
export const PATCH = withAuth<Params>(async (req: NextRequest, { params }) => {
  try {
    ...
    return handleApiError(error, logger.error.bind(logger), { path: "/api/part-statuses/[id]", method: "PATCH" });
  }
...
export const DELETE = withAuth<Params>(async (_req: NextRequest, { params }) => {
  try {
    ...
    return handleApiError(error, logger.error.bind(logger), { path: "/api/part-statuses/[id]", method: "DELETE" });
  }
}, { requiredPermissions: ["parts.manage"] });
