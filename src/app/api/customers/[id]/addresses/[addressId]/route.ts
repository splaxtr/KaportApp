...
export const PATCH = withAuth<Params>(async (req: NextRequest, { params }) => {
  try {
    ...
    return handleApiError(error, logger.error.bind(logger), { path: "/api/customers/[id]/addresses/[addressId]", method: "PATCH" });
  }
...
export const DELETE = withAuth<Params>(async (_req: NextRequest, { params }) => {
  try {
    ...
    return handleApiError(error, logger.error.bind(logger), { path: "/api/customers/[id]/addresses/[addressId]", method: "DELETE" });
  }
}, { requiredPermissions: ["customers.manage"] });
