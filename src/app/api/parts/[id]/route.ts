...
export const PATCH = withAuth<Params>(async (req: NextRequest, { params }) => {
  try {
    ...
    return handleApiError(error, logger.error.bind(logger), { path: "/api/parts/[id]", method: "PATCH" });
  }
...
export const DELETE = withAuth<Params>(async (req: NextRequest, { params, user }) => {
  try {
    ...
    return handleApiError(error, logger.error.bind(logger), { path: "/api/parts/[id]", method: "DELETE" });
  }
}, { requiredPermissions: ["parts.manage"] });
